...
---------------------------------------------
The mobile app's `HttpHabitRepository` (see
`mobile/Habo/lib/repositories/http_habit_repository.dart`) currently expects:

  - GET /<user_id>/habits
      Returns a JSON object with a ``"habits"`` key containing a list of
      objects produced by ``dataclasses.asdict(Habit)``.

      Example response used by the app today:
      {
          "habits": [
              {
                  "habit_name": "Push-ups",
      ...
      The Dart side maps:
        - ``habit_name`` → Habo ``HabitData.title``
        - ``habit_id``   → Habo ``HabitData.id`` (parsed as int if possible)
        - ``is_device``  → controls whether the habit is considered archived

  - POST /<user_id>/habits/add?name=&device=
      Used when the user creates a habit. The route currently returns only a
      message string. The mobile code works around this by re-fetching the
      full list and matching by ``habit_name`` to discover the id.

      TODO (backend): Consider returning the created Habit (including id) so
      the client does not need an extra GET. For example:

          return {
              "habit": asdict(created_habit)
          }, 200
...
def post_add_habit(user_id: int):
    ...
    return {
        "response": f"successfully added Habit {habit_name}"
    }, 200
